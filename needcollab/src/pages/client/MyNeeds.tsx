  ...
  return (
    <div className="container py-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button asChild><Link to="/create"><Plus className="mr-2 h-4 w-4" />Nouveau</Link></Button>
      </div>

      {myNeeds.length === 0 ? (
        <div className="py-16 text-center text-muted-foreground">Aucun besoin publié.</div>
      ) : (
            ...
            <Card key={need.id}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="outline">{statusLabels[need.status]}</Badge>
              ...
              <CardContent>
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-muted-foreground">{need.offersCount} offres · {need.votesCount} votes</span>
                </div>
                <Link to={`/needs/${need.id}`}>
                  <Button variant="link" className="mt-2 p-0">Voir les détails →</Button>
                </Link>
              </CardContent>
            </Card>
          ))}
